!function (n) {
    ...
    !function () {
        ...
        var s = {}, r = {}, c = function (e, t) {
            var n = Date.now();
            for (key in e) s[key] = e[key], void 0 === e[key] && chrome.storage.local.remove(key), key.match(/\+ts$/) || (s[key + "+ts"] = e[key + "+ts"] = n);
            chrome.storage.local.set(e, t)
        }, l = {initcount: 2};
        chrome.storage.local.get(null, function (e) {
            chrome.runtime.lastError ? (l.failed || alert("Google Chrome's storage may be corrupted. Extensions may not work properly.\n\nTry closing and restarting Chrome.\n\nIf that doesn't work, reinstall the browser to fix this problem."), l.failed = !0, console.log("chrome.storage failure"), s = JSON.parse(localStorage.local)) : (s = e, localStorage.local = JSON.stringify(e)), 0 == --l.initcount && l.init()
        }), chrome.storage.sync.get(null, function (e) {
            chrome.runtime.lastError ? (l.failed || alert("Google Chrome's storage may be corrupted. Extensions may not work properly.\n\nTry closing and restarting Chrome.\n\nIf that doesn't work, reinstall the browser to fix this problem."), l.failed = !0, console.log("chrome.storage failure"), r = JSON.parse(localStorage.sync)) : (r = e, localStorage.sync = JSON.stringify(e)), 0 == --l.initcount && l.init()
        }), l.changed = function (e, t) {
            if ("local" == t) {
                var n = {};
                for (key in console.log("localchanged", e), e) s[key] = e[key].newValue, r.sync && r.sync[key] && (r[key] = n[key] = e[key].newValue), console.log("syncsync", key, r.sync && r.sync[key], n);
                console.log("updatesync", Object.keys(n).length, n), Object.keys(n).length && chrome.storage.sync.set(n), l.localChanged(e)
            } else if ("sync" == t) {
                if (e.firstinstalled) {
                    if (!e.firstinstalled.newValue) return void chrome.storage.sync.set(r);
                    r.firstinstalled && e.firstinstalled.newValue > e.firstinstalled.oldValue && (e.firstinstalled.newValue = e.firstinstalled.oldValue, chrome.storage.sync.set({firstinstalled: e.firstinstalled.oldValue}))
                }
                var o = {};
                for (key in console.log("syncchanged", e), e) r[key] = e[key].newValue, r.sync && r.sync[key] && (s[key] = o[key] = e[key].newValue), console.log("synclocal", key, r.sync && r.sync[key], o);
                console.log("updatelocal", Object.keys(o).length, o), Object.keys(o).length && chrome.storage.local.set(o), l.syncChanged(e)
            }
        }, chrome.storage.onChanged.addListener(l.changed), l.init = function () {
            for (key in r.firstinstalled || (r.firstinstalled = Date.now(), r.sync = {firstinstalled: !0}), JSON.parse(k["Smooth Gestures"].settings)) r.sync[key] = !0;
            if (r.sync.gestures = !0, r.sync.customactions = !0, !s.installed) {
                s.installed = Date.now(), s.id = Math.floor(Math.random() * Math.pow(2, 30)).toString(32) + Math.floor(Math.random() * Math.pow(2, 30)).toString(32), s.log = {action: {}}, s.gestures = JSON.parse(k["Smooth Gestures"].gestures);
                ...
                }, s.externalactions = {}, setTimeout(function () {
                    chrome.tabs.create({url: "options.html"})
                }, 1e3)
            }
            for (key in r.firstinstalled > s.installed && (r.firstinstalled = s.installed), r.sync) r.sync[key] && void 0 !== r[key] && (r[key + "+ts"] || 0) >= (s[key + "+ts"] || 0) ? (s[key] = r[key], s[key + "+ts"] = r[key + "+ts"] || Date.now()) : r.sync[key] && void 0 !== s[key] && (s[key + "+ts"] || 0) >= (r[key + "+ts"] || 0) && (r[key] = s[key], r[key + "+ts"] = s[key + "+ts"] || Date.now());
            if (s.version = chrome.runtime.getManifest().version, s.started = Date.now(), s.session = Math.floor(Math.random() * Math.pow(2, 30)).toString(32) + Math.floor(Math.random() * Math.pow(2, 30)).toString(32), o = s.license, s.forceInstallRightclick) {
                var t = screen.availHeight / 2 - 320 / 1.5, n = screen.availWidth / 2 - 375;
            ...
            e.gestures && A(), e.license_expires && e.license_expires.oldValue < Date.now() && !e.license_expires.newValue && c({license_showexpired: !0}), s.license != o ? c({license: o}) : e.license && void 0 !== e.license.oldValue && c({
                license_showactivated: !!o,
                license_showdeactivated: !o && !s.license_showexpired
            }, function () {
            }), e.version && "2.8.1" == s.version && e.version.oldValue && "2.8.1" != e.version.oldValue && c({showNoteUpdated: !0}, function () {
                chrome.tabs.create({url: chrome.runtime.getURL("/options.html#changelog")})
            })
        }, l.syncChanged = function () {
        };
        navigator.platform.indexOf("Win");
        var d = -1 != navigator.platform.indexOf("Mac"), u = -1 != navigator.platform.indexOf("Linux"), m = null,
            h = {}, f = {active: null, prevActive: null, closed: [], tab: {}}, b = {}, w = null, p = null, o = null,
            v = 1745208e3 < Date.now() / 1e3, k = {"Smooth Gestures": {}};
        k["Smooth Gestures"].settings = JSON.stringify({
            ...
            RU: "maximize-window",
            RDLUR: "options"
        }), k.Opera = {}, k.Opera.settings = JSON.stringify({}), k.Opera.gestures = JSON.stringify({
            L: "page-back",
            rRL: "page-back",
            R: "page-forward",
            rLR: "page-forward",
            ...
            lDU: "new-tab-back",
            UL: "parent-dir"
        }), k["All-in-One Gestures"] = {}, k["All-in-One Gestures"].settings = JSON.stringify({}), k["All-in-One Gestures"].gestures = JSON.stringify({
            L: "page-back",
            rRL: "page-back",
            R: "page-forward",
            rLR: "page-forward",
            ...
            LDR: "show-cookies",
            DRD: "options"
        }), k.FireGestures = {}, k.FireGestures.settings = JSON.stringify({}), k.FireGestures.gestures = JSON.stringify({
            L: "page-back",
            R: "page-forward",
            UD: "reload-tab",
            UDU: "reload-tab-full",
        ...
        var I = {
            ...
            "new-tab": function (e, n) {
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    ...
                    "homepage" != s.newTabUrl && (t.url = s.newTabUrl), s.newTabRight && (t.index = e.index + 1), chrome.tabs.create(t, n)
                })
            }, "new-tab-link": function (e, o, i) {
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    for (var t = 0; t < i.links.length; t++) {
                        var n = {openerTabId: e.id, windowId: e.windowId, url: i.links[t].src};
                        s.newTabLinkRight && (n.index = e.index + 1 + t), chrome.tabs.create(n, t == i.links.length - 1 ? o : null)
                    }
                })
            }, "new-tab-back": function (e, o, i) {
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    for (var t = 0; t < i.links.length; t++) {
                        var n = {openerTabId: e.id, windowId: e.windowId, url: i.links[t].src, active: !1};
                        s.newTabLinkRight && (n.index = e.index + 1 + t), chrome.tabs.create(n, t == i.links.length - 1 ? o : null)
                    }
                })
                ...
                chrome.tabs.get(h[t].detail.tabId, function (e) {
                    e.pinned ? n() : s.closeLastBlock ? chrome.windows.getAll({populate: !0}, function (e) {
                        1 == e.length && 1 == e[0].tabs.length ? chrome.tabs.update(h[t].detail.tabId, {url: "homepage" != s.newTabUrl ? s.newTabUrl : void 0}, n) : chrome.tabs.remove(h[t].detail.tabId, n)
                ...
                chrome.tabs.get(h[e].detail.tabId, function (n) {
                    chrome.tabs.query({windowId: n.windowId}, function (e) {
                        var t = null;
                        for (i = e.length - 1; 0 <= i; i--) if (t = e[(n.index + i) % e.length].id, C(t)) return void chrome.tabs.update(t, {active: !0}, o);
                        o()
                    })
                })
                ...
                chrome.tabs.get(h[e].detail.tabId, function (n) {
                    chrome.tabs.query({windowId: n.windowId}, function (e) {
                        var t = null;
                        for (i = 1; i <= e.length; i++) if (t = e[(n.index + i) % e.length].id, C(t)) return void chrome.tabs.update(t, {active: !0}, o);
                        o()
                    })
                })
                ...
                chrome.tabs.get(h[e].detail.tabId, function (n) {
                    chrome.tabs.query({windowId: n.windowId}, function (t) {
                        chrome.windows.create({tabId: n.id, focused: !0, incognito: n.incognito}, function (e) {
                            for (i = n.index + 1; i < t.length; i++) chrome.tabs.move(t[i].id, {
                                windowId: e.id,
                                index: i - n.index
                            })
                        })
                ...
                O(e, function () {
                    ...
                    if (e = document.querySelector("link[rel=next][href]")) location.href = e.href; else if (e = document.querySelector("a[rel=next][href]")) location.href = e.href; else {
                        ...
                        for (var t = 0; t < e.length; t++) if (e[t].innerText.match(/(next|下一页|下页)/i)) return void (location.href = e[t].href);
                        e = document.querySelectorAll("a[href]");
                        for (t = 0; t < e.length; t++) if (e[t].innerText.match(/(>|›)/i)) return void (location.href = e[t].href)
                    }
                }, t)
                ...
                O(e, function () {
                    ...
                    if (e = document.querySelector("link[rel=prev][href]")) location.href = e.href; else if (e = document.querySelector("a[rel=prev][href]")) location.href = e.href; else {
                        e = document.querySelectorAll("a[href]");
                        for (var t = 0; t < e.length; t++) if (e[t].innerText.match(/(prev|上一页|上页)/i)) return void (location.href = e[t].href);
                        e = document.querySelectorAll("a[href]");
                        for (t = 0; t < e.length; t++) if (e[t].innerText.match(/(<|‹)/i)) return void (location.href = e[t].href)
                    }
                }, t)
                ...
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    chrome.windows.get(e.windowId, function (e) {
                        b[e.id] || (b[e.id] = {}), chrome.windows.update(e.id, {state: "fullscreen" != e.state ? "fullscreen" : b[e.id].prevstate || "normal"}, t), b[e.id].prevstate = e.state
                    })
                })
            }, "minimize-window": function (e, t) {
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    chrome.windows.get(e.windowId, function (e) {
                        b[e.id] || (b[e.id] = {}), chrome.windows.update(e.id, {state: "minimized" != e.state ? "minimized" : b[e.id].prevstate || "normal"}, t), b[e.id].prevstate = e.state
                    })
                })
            }, "maximize-window": function (e, t) {
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    chrome.windows.get(e.windowId, function (e) {
                        b[e.id] || (b[e.id] = {}), chrome.windows.update(e.id, {state: "maximized" != e.state ? "maximized" : "normal"}, t), b[e.id].prevstate = e.state
                    })
                })
            ...
            chrome.tabs.update(c.id, {active: !0}, function () {
                chrome.tabs.executeScript(c.id, {code: 'var ssfo=document.body.style.overflow;document.body.style.overflow="hidden";var ssf={top:document.body.scrollTop,left:document.body.scrollLeft,height:document.body.scrollHeight,width:document.body.scrollWidth,screenh:window.innerHeight,screenw:window.innerWidth,overflow:ssfo};ssf;'}, function (e) {
                    var t = e[0], n = document.createElement("canvas");
                    n.height = Math.min(t.height, 32768), n.width = Math.min(t.width, 32768);
                    var o = document.createElement("img"), i = n.getContext("2d"), a = 0, r = 0, s = function () {
                        chrome.tabs.executeScript(c.id, {code: "document.body.scrollTop=" + a * t.screenh + ";document.body.scrollLeft=" + r * t.screenw + ";"}, function () {
                            setTimeout(function () {
                    ...
                    o.addEventListener("load", function () {
                        i.drawImage(o, 0, 0, o.width, o.height, Math.min(r * o.width, t.width - t.screenw), Math.min(a * o.height, t.height - t.screenh), o.width, o.height), a + 1 < n.height / t.screenh ? (a++, s()) : r + 1 < n.width / t.screenw ? (a = 0, r++, s()) : chrome.tabs.executeScript(c.id, {code: "document.body.scrollTop=" + t.top + ";document.body.scrollLeft=" + t.left + ';document.body.style.overflow="' + t.overflow + '"'}, function () {
                            l(S(n.toDataURL()))
                        })
                    }), s()
                })
        ...
        L["clone-tab"] = function (e, t, n) {
            ...
            m ? (m.postMessage({
                ...
                timestamp: Date.now()
            }), t()) : h[e].postMessage({action: {id: "zoom-in-hack"}}, t)
        }, L["zoom-out"] = function (e, t) {
            m ? (m.postMessage({
                key: {keys: ["-"], mod: [d ? "meta" : "ctrl"]},
                timestamp: Date.now()
            }), t()) : h[e].postMessage({action: {id: "zoom-out-hack"}}, t)
        }, L["zoom-zero"] = function (e, t) {
            m ? (m.postMessage({
                key: {keys: ["0"], mod: [d ? "meta" : "ctrl"]},
                timestamp: Date.now()
            }), t()) : h[e].postMessage({action: {id: "zoom-zero-hack"}}, t)
            ...
            chrome.tabs.get(h[e].detail.tabId, function (e) {
                ...
                var n = null;
                (n = 3 < t.length ? t.slice(0, t.length - 1).join("/") + "/" : t.join("/") + "/") ? chrome.tabs.update(e.id, {url: n}, o) : o()
            })
            ...
            chrome.tabs.get(h[e].detail.tabId, function (e) {
                for (var t = 0; t < o.images.length; t++) chrome.tabs.create({
                    url: o.images[t].src,
                    openerTabId: e.id,
                    windowId: e.windowId
                }, n)
            ...
            O(e, "window.alert('Cookies stored by this host or domain:\\n'+('\\n'+document.cookie).replace(/; /g,';\\n').replace(/\\n(.{192})([^\\n]{5})/gm,\"\\n$1\\n        $2\").replace(/\\n(.{100})([^\\n]{5})/gm,\"\\n$1\\n        $2\"));", t)
            ...
            if (!n.selection) return t();
            O(e, "window.find('" + n.selection.replace(/[\\"']/g, "\\$&") + "', false, false, true, false, true, true);", t)
        }, L["toggle-bookmark"] = function (e, n) {
            M(["bookmarks"], function () {
                chrome.tabs.get(h[e].detail.tabId, function (t) {
                    chrome.bookmarks.search(t.url, function (e) {
                        e.length <= 0 ? chrome.bookmarks.create({
                            parentId: "2",
                            title: t.title,
                            url: t.url
                        }, n) : chrome.bookmarks.remove(e[0].id, n)
                    })
                })
            })
        ...
        chrome.runtime.onMessage.addListener(function (e, t, n) {
            ...
            if (e.getgestures) {
                if (!w) return $.get(chrome.runtime.getURL("js/gestures.js"), null, function (e) {
                    w = "window.SGextId='" + chrome.runtime.id + "';\n" + e, n({gestures: w})
                }), !0;
                n({gestures: w})
            } else if (e.storage) {
                var o = ["gestures", "validGestures", "contextOnLink", "holdButton", "trailBlock", "trailColor", "trailWidth", "trailLegacy", "selectToLink"],
                    a = {};
                ...
                if (r.name && r.actions) {
                    if (0 < r.actions.length) for (s.externalactions[t.id] = r, i = 0; i < s.externalactions[t.id].actions.length; i++) I[t.id + "-" + s.externalactions[t.id].actions[i].id] = s.externalactions[t.id].actions[i].context; else delete s.externalactions[t.id];
        ...
        var t = function (e) {
            if (e.sender && e.sender.tab && e.detail.id) {
                var t = e.sender.tab, n = e.detail.id;
                h[n] = e, h[n].onMessage.addListener(function (t, n) {
                    if (console.log("content_message", JSON.stringify(n)), n.selection && 0 < n.selection.length && s.gestures["s" + n.gesture] ? n.gesture = "s" + n.gesture : n.links && 0 < n.links.length && s.gestures["l" + n.gesture] ? n.gesture = "l" + n.gesture : n.images && 0 < n.images.length && s.gestures["i" + n.gesture] && (n.gesture = "i" + n.gesture), n.gesture && s.gestures[n.gesture]) {
                        if (v) return void J();
                        var e = s.gestures[n.gesture];
                        console.log("gesture", n.gesture, e), p && clearTimeout(p.timeout), p = null, "r" == n.gesture[0] && (p = {
                            rocker: !0,
                        ...
                        }), p && n.buttonDown && (p.buttonDown = n.buttonDown), p && n.startPoint && (p.startPoint = n.startPoint);
                        var o = p ? function () {
                            chrome.tabs.query({active: !0, lastFocusedWindow: !0}, function (e) {
                                if (p && e.length) for (t in p.tabId = e[0].id, h) e[0].id == h[t].detail.tabId && h[t].postMessage({chain: p})
                            })
                        } : function () {
                        };
                        try {
                            if (L[e]) L[e].call(null, t, o, n); else if (s.externalactions[e.substr(0, 32)]) chrome.runtime.sendMessage(e.substr(0, 32), {doaction: e.substr(33)}); else if (s.customactions[e]) {
                                var i = s.customactions[e];
                                "page" == i.env && O(t, i.code, o)
                            }
                        } catch (e) {
                        }
                        s.log.action[e] || (s.log.action[e] = {}), s.log.action[e][n.gesture] || (s.log.action[e][n.gesture] = {count: 0}), s.log.action[e][n.gesture].count += 1, s.log.line || (s.log.line = {
                            distance: 0,
                            segments: 0
                        }), n.line && (s.log.line.distance += n.line.distance, s.log.line.segments += n.line.segments), c({log: s.log})
                    }
                    if (n.syncButton && (p && (p.buttonDown || (p.buttonDown = {}), p.buttonDown[n.syncButton.id] = n.syncButton.down), setTimeout(function () {
                        chrome.tabs.query({active: !0, lastFocusedWindow: !0}, function (e) {
                            for (t in h) e[0].id == h[t].detail.tabId && h[t].postMessage({syncButton: n.syncButton})
                        })
                    }, 20)), n.closetab && chrome.tabs.get(h[t].detail.tabId, function (e) {
                        ...
                        if (m) m.postMessage({
                            ...
                            var a = screen.availHeight / 2 - 320 / 1.5, r = screen.availWidth / 2 - 375;
                            window.open("rightclick.html", "rightclick", "width=750,height=320,top=" + a + ",left=" + r)
                        }
                    }
                }.bind(null, n)), h[n].onDisconnect.addListener(function () {
                    delete h[n]
                });
                var o = {enable: !0};
                ...
                for (var a = 0; s.blacklist && a < s.blacklist.length; a++) new RegExp("^(.+\\.)?" + s.blacklist[a].replace(".", "\\.") + "$").test(i) && (o.enable = !1);
                h[n].postMessage(o), _(t.id)
            }
        }, O = function (e, t, n, o) {
            h[e] && ("function" == typeof n && (o = n, n = void 0), void 0 === n && (n = []), "object" == typeof n && n.constructor === Array || (n = [n]), console.log("runJS:", t), "string" == typeof t && (t = "(function(){" + t + "})()"), "function" == typeof t && (t = "(" + t.toString() + ")(" + n.map(function (e) {
                return JSON.stringify(e)
            }).join(",") + ")"), t = '(function(){if(window.SG && window.SG.isId("' + e + '")){return ' + t + "}})()", chrome.tabs.executeScript(h[e].sender.tab.id, {
                code: t,
                allFrames: !0,
                matchAboutBlank: !0
            }, function (e) {
        ...
        chrome.tabs.onActivated.addListener(function (e) {
            ...
            e != chrome.windows.WINDOW_ID_NONE && (b[e] || (b[e] = {}), b[e].focused = Date.now(), chrome.tabs.query({
                active: !0,
                lastFocusedWindow: !0
            }, function (e) {
                e.length && n(e[0].id)
        ...
        var z = function (d, u) {
            chrome.tabs.get(d, function (e) {
                if (!chrome.runtime.lastError) {
                    if ("https://smoothgesturesplus.com/thanks" == e.url && V(), u && u.url && (e.url = u.url, e.title = u.url), "http://www.google.com/?index=" == e.url.substr(0, 29)) {
                        var t = e.url.split("#"), n = t[0].split("?"), o = t[1].substr(4).split(":--:"),
                            i = (o[0], JSON.parse(unescape(o[1]))), a = JSON.parse(unescape(o[2])),
                            r = 1 * n[1].substr(6);
                        e.url = "";
                    ...
                    0 <= l ? (c.history = c.history.slice(0, l + 1), c.titles = c.titles.slice(0, l + 1), c.titles[l] = e.title) : (c.history.push(e.url), c.titles.push(e.title), 10 < c.history.length && (c.history.shift(), c.titles.shift())), "loading" == e.status && (chrome.pageAction.setIcon({
                        tabId: d,
                        path: chrome.runtime.getURL("/img/pageaction.png")
                    }), chrome.pageAction.setTitle({
                        tabId: d,
                        title: "Smooth Gestures"
                    }), chrome.pageAction.show(d)), "complete" == e.status && setTimeout(function () {
                        _(d)
                    }, 100)
                }
            })
        };
        chrome.tabs.onUpdated.addListener(z), chrome.tabs.onMoved.addListener(z), chrome.tabs.onAttached.addListener(z), chrome.tabs.onRemoved.addListener(function (e) {
        ...
        var A = function () {
            var e = {};
            for (g in s.gestures) if ("l" != g[0] && "i" != g[0] && "s" != g[0] || (g = g.substr(1)), "k" == g[0]) {
                e.k || (e.k = {});
                var t = g.substr(1, 4);
            ...
            var t = null;
            for (id in h) if (e == h[id].detail.tabId) {
            ...
            for (id in h) {
                ...
                s[e] || (s[e] = {root: !1, frames: 0}), h[id].detail.frame ? s[e].frames += 1 : s[e].root = !0
            }
            chrome.windows.getAll({populate: !0}, function (e) {
                ...
                for (j = 0; j < e.length; j++) {
                    ...
                    for (t[n.id] = [], i = 0; i < n.tabs.length; i++) {
                        var o = n.tabs[i], a = null;
                        s[o.id] ? (a = s[o.id], delete s[o.id]) : a = {
                            root: !1,
                            frames: 0
                        }, a.goodurl = "chrome://" != o.url.substr(0, 9) && "chrome-extension://" != o.url.substr(0, 19) && "https://chrome.google.com/" != o.url.substr(0, 26), a.title = o.title, a.url = o.url, a.tabStatus = o.status, a.tabId = o.id, t[n.id].push(a)
                    }
                    t.extra = s
                }
                r(t)
            ...
            C(e);
            C(e) ? t("working") : chrome.tabs.get(e, function (e) {
                e && e.url.match(/^(chrome:\/\/|chrome-extension:\/\/|https:\/\/chrome\.google\.com|file:\/\/|[^:\/]+:[^:\/]+)/) ? t("unable") : t("broken")
            })
        }, _ = function (t) {
            G(t, function (e) {
                "unable" == e ? (chrome.pageAction.setIcon({
                    tabId: t,
                    path: chrome.runtime.getURL("/img/pageaction-unable.png")
                }), chrome.pageAction.setTitle({
                    tabId: t,
                    title: "Chrome blocks Gestures on this page"
                })) : "broken" == e ? (chrome.pageAction.setIcon({
                    tabId: t,
                    ...
                    tabId: t,
                    path: chrome.runtime.getURL("/img/pageaction.png")
                }), chrome.pageAction.setTitle({tabId: t, title: "Smooth Gestures"})), chrome.pageAction.show(t)
            })
        }, J = function () {
        ...
        var V = function (t) {
            navigator.onLine ? (delete q.ping, $.ajax({
                ...
                "string" == typeof e && (e = JSON.parse(e)), e.alert && alert(e.alert), e.checkupdate && J(), 3 == e.code && (v = !0), e.invalidtoken && r.token && (r.invalidtoken = r.token, chrome.storage.sync.set({invalidtoken: r.invalidtoken}), delete r.token, chrome.storage.sync.remove("token")), e.settoken && (r.token = e.settoken, chrome.storage.sync.set({token: r.token})), o = -1 != ["full", "1yrmul", "6mnmul", "1mnmul", "2wkmul", "1wkmul", "1yr1cl", "1mn1cl", "1wk1cl", "free"].indexOf(e.licenseid) ? e.licenseid : null, c({
                    license: o,
            ...
            m || chrome.permissions.contains({permissions: ["nativeMessaging"]}, function (e) {
                if (console.log("connectNative", e), e) {
                    B = !0;
                    try {
                        (m = chrome.runtime.connectNative("com.smoothgesturesplus.extras")).onMessage.addListener(function (e) {
                            console.log("nativemessage", e), m && (n && (clearTimeout(n), n = null, t()), e.version && (m.version = e.version))
                        }), m.onDisconnect.addListener(function () {
                            m && (m = null, console.log("nativedisconnect: retryTimeout: ", o), clearTimeout(n), n = null, 0 < o && o < 6e4 && setTimeout(P, o, 1.01 * o))
                        });
                        var t = function () {
                            for (var e = chrome.extension.getViews(), t = 0; t < e.length; t++) "/rightclick.html" == e[t].location.pathname && e[t].close()
                        }, n = setTimeout(t, 1e3)
                    } catch (e) {
                        console.error("connectNative", B, e), B && setTimeout(function () {
        ...
        var W = function () {
            chrome.windows.getAll({populate: !0}, function (e) {
                for (x in e) for (y in b[e[x].id] = {}, e[x].tabs) !function (e) {
                    f.tab[e.id] = {
                        winId: e.windowId,
                        index: e.index,
                        history: [e.url],
                        ...
                        chrome.tabs.executeScript(e.id, {allFrames: !0, matchAboutBlank: !0, file: "js/gestures.js"})
            ...
            for (id in s.externalactions) for (i = 0; i < s.externalactions[id].actions.length; i++) I[id + "-" + s.externalactions[id].actions[i].id] = s.externalactions[id].actions[i].context;
            ...
            }(chrome.runtime.getManifest().version, s.version) && c({
                version: chrome.runtime.getManifest().version,
                updated: Date.now()
            }), s.externalactions) delete s.externalactions[id], c({externalactions: s.externalactions}), chrome.runtime.sendMessage(id, {getexternalactions: !0});
            setTimeout(W, 0), chrome.tabs.query({active: !0, lastFocusedWindow: !0}, function (e) {
                e.length && (f.active = e[0].id)
            }), V()
        };
        chrome.runtime.onUpdateAvailable.addListener(function (e) {
            chrome.runtime.reload()
        }), window.defaults = k, window.categories = {
        ...
        }, window.contexts = I, window.ensure_permissions = M, window.continue_permissions = function () {
            setTimeout(function () {
                R && R(), setTimeout(function () {
                    chrome.runtime.reload()
                }, 500)
            }, 0)
        }, window.getTabStates = N, window.getTabStatus = G, window.refreshPageAction = _, window.ping = V, window.connectNative = P, window.disconnectNative = function (e) {
