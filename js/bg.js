!function (n) {
    ...
    !function () {
        ...
        chrome.storage.local.get(null, function (e) {
            chrome.runtime.lastError ? (l.failed || alert("Google Chrome's storage may be corrupted. Extensions may not work properly.\n\nTry closing and restarting Chrome.\n\nIf that doesn't work, reinstall the browser to fix this problem."), l.failed = true, console.log('chrome.storage failure'), s = JSON.parse(localStorage.local)) : (s = e, localStorage.local = JSON.stringify(e)), 0 == --l.initcount && l.init()
        }), chrome.storage.sync.get(null, function (e) {
            chrome.runtime.lastError ? (l.failed || alert("Google Chrome's storage may be corrupted. Extensions may not work properly.\n\nTry closing and restarting Chrome.\n\nIf that doesn't work, reinstall the browser to fix this problem."), l.failed = true, console.log('chrome.storage failure'), r = JSON.parse(localStorage.sync)) : (r = e, localStorage.sync = JSON.stringify(e)), 0 == --l.initcount && l.init()
            ...
            for (key in r.firstinstalled || (r.firstinstalled = Date.now(), r.sync = {firstinstalled: true}), JSON.parse(k['Smooth Gestures'].settings)) r.sync[key] = true;
            ...
            e.gestures && A(), e.license_expires && e.license_expires.oldValue < Date.now() && !e.license_expires.newValue && c({license_showexpired: true}), s.license != o ? c({license: o}) : e.license && void 0 !== e.license.oldValue && c({
                license_showactivated: !!o,
                license_showdeactivated: !o && !s.license_showexpired
            }, function () {
            }), e.version && '2.8.1' == s.version && e.version.oldValue && '2.8.1' != e.version.oldValue && c({showNoteUpdated: true}, function () {
        ...
        var p = null
        var o = null
        var v = 1745208e3 < Date.now() / 1e3, k = {'Smooth Gestures': {}};
        ...
        k.Opera = {}
        k.Opera.settings = JSON.stringify({}), k.Opera.gestures = JSON.stringify({
            L: 'page-back',
            rRL: 'page-back',
            R: 'page-forward',
            rLR: 'page-forward',
        ...
        var L = {
            'new-tab': function (e, n) {
                ...
                chrome.tabs.get(h[e].detail.tabId, function (e) {
                    for (var t = 0; t < i.links.length; t++) {
                        var n = {openerTabId: e.id, windowId: e.windowId, url: i.links[t].src, active: false};
                        s.newTabLinkRight && (n.index = e.index + 1 + t), chrome.tabs.create(n, t == i.links.length - 1 ? o : null)
                    }
                })
                ...
                chrome.tabs.get(h[t].detail.tabId, function (e) {
                    e.pinned ? n() : s.closeLastBlock ? chrome.windows.getAll({populate: true}, function (e) {
                ...
                chrome.tabs.get(h[e].detail.tabId, function (n) {
                    chrome.tabs.query({windowId: n.windowId}, function (e) {
                        var t = null;
                        for (i = e.length - 1; 0 <= i; i--) if (t = e[(n.index + i) % e.length].id, C(t)) return void chrome.tabs.update(t, {active: true}, o);
                        o()
                    })
                })
                ...
                chrome.tabs.get(h[e].detail.tabId, function (n) {
                    chrome.tabs.query({windowId: n.windowId}, function (e) {
                        var t = null;
                        for (i = 1; i <= e.length; i++) if (t = e[(n.index + i) % e.length].id, C(t)) return void chrome.tabs.update(t, {active: true}, o);
                        o()
                    })
                })
        ...
        var T = function (e, t) {
            ...
            o.initMouseEvent('click', true, false, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null), n.dispatchEvent(o)
        };
        ...
        var t = function (e) {
            if (e.sender && e.sender.tab && e.detail.id) {
                ...
                h[n] = e, h[n].onMessage.addListener(function (t, n) {
                    if (console.log('content_message', JSON.stringify(n)), n.selection && 0 < n.selection.length && s.gestures['s' + n.gesture] ? n.gesture = 's' + n.gesture : n.links && 0 < n.links.length && s.gestures['l' + n.gesture] ? n.gesture = 'l' + n.gesture : n.images && 0 < n.images.length && s.gestures['i' + n.gesture] && (n.gesture = 'i' + n.gesture), n.gesture && s.gestures[n.gesture]) {
                        ...
                        console.log('gesture', n.gesture, e), p && clearTimeout(p.timeout), p = null, 'r' == n.gesture[0] && (p = {
                            ...
                            wheel: true, timeout: setTimeout(function () {
                                p = null
                            }, 2e3)
                ...
                for (var a = 0; s.blacklist && a < s.blacklist.length; a++) new RegExp('^(.+\\.)?' + s.blacklist[a].replace('.', '\\.') + '$').test(i) && (o.enable = false);
        ...
        var W = function () {
            ...
            for (id in function (e, t) {
                ...
                version: chrome.runtime.getManifest().version,
                updated: Date.now()
            }), s.externalactions) delete s.externalactions[id], c({externalactions: s.externalactions}), chrome.runtime.sendMessage(id, {getexternalactions: true});
            setTimeout(W, 0), chrome.tabs.query({active: true, lastFocusedWindow: true}, function (e) {
