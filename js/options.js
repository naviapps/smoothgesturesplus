!function () {
    ...
    var n = function () {
        ...
        var a = ['gestures', 'customactions', 'blacklist', 'contextOnLink', 'holdButton', 'newTabLinkRight', 'newTabRight', 'newTabUrl', 'selectToLink', 'trailBlock', 'trailColor', 'trailWidth', 'trailLegacy'];
        ...
        $(function () {
            ...
            $.fx.off = !0, pages.init(), $('.upgradebutton').remove(), d.init(), $('.addgesture').html('<span>+</span> ' + c('options_button_startaddgesture')).click(d.open.bind(null, null)), function () {
                ...
                $('.page[page=custom] .pagetitle').append($("<div id=addaction class='button gray'>").html('<span>+</span> ' + c('options_button_addcustomaction')).on('click', a)), m()
            }(), n(), function () {
                s();
        ...
        chrome.runtime.getBackgroundPage(function (e) {
            for (var t = 0; t < g.length; t++) {
                ...
                for (var s = 0; s < g[t].actions.length; s++) {
                    var o = g[t].actions[s], a = e.contexts[o];
                    $('.actiongroup.disabled', n).append($('<div class=action>').attr('action', o).attr('sectionindex', s).append('<div class=gestures>').append($("<div class='button gray addactiongesture' tabindex=0>+</div>").on('click', d.open.bind(null, o))).append(a ? $('<img class=context>').attr('src', '/img/icon-' + ('l' == a ? 'link' : 'i' == a ? 'image' : 's' == a ? 'selection' : '') + '.png') : null).append($('<div class=headtitle>').text(c('action_' + o))).append($('<p class=sub>').text(c('descrip_' + o))).append('<div class=clear>'))
                }
            }
            n = $('.page[page=custom]');
            $('.action', n).remove();
            ...
            for (var o in l.customactions) {
                var i = l.customactions[o];
                a = e.contexts[o];
                $('.actiongroup.disabled', n).append($('<div class=action>').attr('action', o).attr('sectionindex', t).append($("<div class='button gray delcustomaction' tabindex=0>&times;</div>").on('click', r.bind(null, o))).append($("<div class='button gray editcustomaction' tabindex=0>edit</div>").on('click', u.bind(null, o))).append('<div class=gestures>').append($("<div class='button gray addactiongesture' tabindex=0>+</div>").on('click', d.open.bind(null, o))).append(a ? $('<img class=context>').attr('src', '/img/icon-' + ('l' == a ? 'link' : 'i' == a ? 'image' : 's' == a ? 'selection' : '') + '.png') : null).append($('<div class=headtitle>').text(i.title)).append($('<p class=sub>').text(i.descrip)).append('<div class=clear>')), t++
            }
            h()
        })
    }, h = function () {
