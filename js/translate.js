...
chrome.i18n.getAcceptLanguages(function (e) {
    ...
    var t = $("<optgroup>").attr("label", "Detected Languages"), a = $("<optgroup>").attr("label", "Other Languages");
    ...
    $("body").append($("<div>").attr("id", "languages").append($("<div>").text("Translate to: ").append($("<select>").append(t).append(a).change(function (e) {
        language = $(this).val(), loadpage()
    })))), $("body").append($("<div>").attr("id", "generatediv").append($("<input>").attr("type", "button").val("Submit >").click(function () {
        var e = new Date,
            t = e.getUTCFullYear().toString() + (e.getUTCMonth() + 1 < 10 ? "0" : "") + (e.getUTCMonth() + 1).toString() + (e.getUTCDate() < 10 ? "0" : "") + e.getUTCDate().toString(),
            a = {};
        for (id in src) {
            var i = $("#edit-" + id).val(), r = res[id] ? res[id].message : null;
            if (i && "" != i && i != r) {
                var n = JSON.parse(JSON.stringify(src[id]));
                n.message = i, n.update = t, a[id] = n
            }
        }
        "{}" != JSON.stringify(a) ? ($("#generatediv input").val("Uploading..."), $.ajax({
            url: "http://fujan.name/smoothgestures/translate.php",
            type: "post",
            data: {key: "smooth", lang: language, tran: JSON.stringify(a)},
            success: function (e) {
                $("#generatediv input").val("Submit >"), "success" == e ? setTimeout(function () {
                    alert("Translation Sent. Thanks!\n\nFeel free to email smoothgestures@fujan.name to notify the developer that you have submitted a translation.")
                }, 0) : setTimeout(function () {
                    alert("Error Sending Translation")
                }, 0)
            },
            error: function () {
                $("#generatediv input").val("Submit >"), setTimeout(function () {
                    alert("Error Sending Translation")
                }, 0)
            }
...
var loadpage = function () {
    ...
    for (tree = {
        ...
        complete: [],
        old: []
    }, i = 0; i < words.length; i++) words[i].res ? words[i].src ? tree.complete.push(words[i]) : tree.old.push(words[i]) : tree.empty.push(words[i]);
    for (s in tree) {
        var t = {};
    ...
    for (c in tree.complete) {
        ...
        e[c] || (e[c] = $("<div>").attr("class", "translatetable").append($("<div>").attr("class", "tabletitle").text(c))), e[c].append(completedwords(t))
    }
    $("#translateroot").remove();
    var a = $("<div>").attr("id", "translateroot"), r = [];
    for (c in e) r.push(c);
