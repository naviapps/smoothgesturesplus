...
$('body')
      ...
      .append(
        $('<div>')
          ...
          .append(
            $('<div>')
              .attr('class', 'title')
              .text(getText('options_contents_actions')),
          )
          .append($('<div>').attr('id', 'contentsactions'))
          .append(
            $('<div>')
              .attr('class', 'title')
              .text(getText('options_contents_settings')),
          )
          ...
          .append(
            $('<ul>')
              .append(
                $(
                  '<li id="translatemess">Help <a id="translatelink" href="translate.html">Translate SG</a></li>',
                ),
              )
              .append(
                $(
                  '<li id="supportmess">Find a bug? <a id="supportlink" href="http://code.google.com/p/smoothgestures-chromium/issues/list">Report it</a></li>',
                ),
              ),
          ),
      ),
  ...
  .append(
    $(
      '<h1 id="optionstitle"><img src="im/icon48.png"/> ' +
        getText('name') +
        ': ' +
        getText('options_title') +
        '</h1>',
    ),
  )
  .append(
...
if (bg.settings.sendStats == undefined)
  $('#optionstitle').after(
    $('<div>')
      .attr('id', 'note_sendStats')
      .attr('class', 'note green')
      .append(
        $('<div>')
          .attr('class', 'close')
          .attr('tabindex', 0)
  ...
  $('#optionstitle').after(
    $('<div>')
      ...
      .append(
        $('<input>')
          .attr('type', 'button')
          .val(getText('options_note_hidepageaction_button'))
          .css({
            'min-width': '5em',
            'font-weight': 'bold',
            float: 'right',
...
if (
  !bg.settings.hideNoteTranslate &&
  ['en', 'en-US', 'en-GB'].indexOf(navigator.language) == -1
)
  $('#optionstitle').after(
    $('<div>')
      .attr('id', 'note_translate')
      .attr('class', 'note green')
      .append(
        $('<div>')
          .attr('class', 'close')
      ...
      .append(
        $('<p>').html(
          getText(
            'options_note_translate',
            "<a href='/translate.html'>" +
              getText('options_note_translate_link') +
              '</a>',
          ),
        ),
      )
  ...
  $('#optionstitle').after(
    $('<div>')
      ...
      .append(
        $('<input>')
          .attr('type', 'button')
          .val(getText('options_note_print_button'))
          .css({
            'min-width': '5em',
            'font-weight': 'bold',
            float: 'right',
...
if (!bg.settings.hideNoteWelcome)
  $('#optionstitle').after(
    $('<div>')
      .attr('id', 'note_welcome')
      .attr('class', 'note clear')
      .append(
        $('<div>')
          .attr('class', 'close')
          .attr('tabindex', 0)
      ...
      .append(
        $('<p>')
          .text(getText('options_welcome_0'))
          .css({ margin: '0 0 .7em', 'font-weight': 'bold' }),
      )
      .append(
        $('<ul>')
          .css({ margin: '0' })
          .append(
            $('<li>')
              .text(getText('options_welcome_1'))
              .css({ 'margin-bottom': '.7em' }),
          )
          .append(
            $('<li>')
              .text(getText('options_welcome_2'))
              .css({ 'margin-bottom': '.7em' }),
          )
          .append(
            $('<li>')
              .text(getText('options_welcome_3'))
              .css({ 'margin-bottom': '.7em' }),
          ),
      )
      .append($('<div>').attr('class', 'clearall')),
...
if (
  !bg.settings
    .hideNoteRemindRate /* && new Date() - new Date(bg.getValue('date_first')) > 30*24*60*60*1000*/
) {
  $('#optionstitle').after(
    $('<div>')
      .attr('id', 'note_remindrate')
      .attr('class', 'note green')
      .append(
        $('<div>')
          .attr('class', 'close')
      ...
      .append(
        $('<p>').html(
          getText(
            'options_note_remindrate',
            "<a href='https://chrome.google.com/webstore/detail/lfkgmnnajiljnolcgolmmgnecgldgeld'>" +
              getText('options_note_remindrate_link') +
              '</a>',
          ),
        ),
      )
    ...
    $('<div>')
      ...
      .text(
        'You have gestured approximately ' +
          ('' + (bg.log.line ? bg.log.line.distance : 0) * 0.000254).substr(
            0,
            8,
          ) +
          ' meters.',
...
var generateTable = function (cat) {
  ...
  if (bg.categories[cat].actions) {
    ...
    for (i = 0; i < bg.categories[cat].actions.length; i++) {
      var t = empty
      for (g in bg.gestures)
        if (bg.gestures[g] == bg.categories[cat].actions[i]) {
          t = table
          break
        }
    ...
    if (empty.children().length > 0)
      table.append(
        actionGroup(
          chrome.i18n.getMessage('options_moreactions') +
            ' (' +
    ...
    $('.tabletitle', table).append(
      $('<div>')
        .attr('id', 'addactionbutton')
        .html(
          '<span>+</span> ' +
    ...
    for (id in bg.externalActions) {
      table.append(
        $('<div>')
          ...
          .text(bg.externalActions[id].name),
      )
    ...
    table
      ...
      .append(
        settingsGroup(
          getText('setting_group_context'),
          'showContextSettings',
          $('<div>').append(rowContextOnLink()),
        ),
      )
      .append(
        settingsGroup(
          getText('setting_group_assorted'),
          'showAssortedSettings',
          $('<div>')
...
var actionGroup = function (title, label, show, rows) {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'actiongrouptitle')
        .text(title)
        .append(
          $('<a>')
            .attr('href', '#')
            .text(
              !bg.settings['hide-' + label]
...
var generateGRow = function (action) {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('id', 'action-' + action)
        .attr('class', 'gestureset'),
    )
    .append(
      $('<div>')
      ...
      bg.contexts[action]
        ? $('<img>')
            ...
            .attr(
              'src',
              'im/icon-' +
                (bg.contexts[action] == 'l'
                  ? 'link'
                  : bg.contexts[action] == 'i'
                    ? 'image'
                    : bg.contexts[action] == 's'
                      ? 'selection'
                      : '') +
                '.png',
            )
    ...
    .append(
      $('<div>')
        .attr('class', 'actiontitle')
        .text(getText('action_' + action)),
    )
    .append(
      $('<div>')
        .attr('class', 'actiondescrip')
        .text(getText('descrip_' + action)),
    )
    .append($('<div>').attr('class', 'clearall'))
}
...
var generateCustomRow = function (id) {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'actiontitle')
        .text(bg.customActions[id].title)
        .append(
          $('<span>')
    ...
    .append(
      $('<div>')
        .attr('class', 'actiondescrip')
        .text(bg.customActions[id].descrip),
    )
    .append($('<div>').attr('class', 'clearall'))
}
var editCustom = function (id) {
  ...
  var edit = $('<div>')
    ...
    .append(
      $('<select>')
        .attr('class', 'cxtcustom')
        .append($('<option>').text('starting anywhere').val(''))
        .append(
          $('<option>')
            .text(chrome.i18n.getMessage('context_on_link'))
            .val('l'),
        )
        .append(
          $('<option>')
            .text(chrome.i18n.getMessage('context_on_image'))
            .val('i'),
        )
        .append(
          $('<option>')
            .text(chrome.i18n.getMessage('context_with_selection'))
            .val('s'),
        ),
    )
    .append(
      $('<select>')
        .attr('class', 'envcustom')
    ...
    .append(
      $('<div>')
        .attr('class', 'sharecustom')
        .append(
          $('<input type=checkbox>')
            .attr('id', 'share-' + id)
            .attr('checked', a.share),
        )
        .append(
          $('<label>')
            .attr('for', 'share-' + id)
    ...
    .append(
      $('<input type=text>').attr('class', 'descripcustom').val(a.descrip),
    )
  ...
  return $('<div>')
    ...
    .append(
      rows
        .attr('id', label + 'Group')
        .attr('class', 'grouprows')
        .css({ display: bg.settings[label] ? 'block' : 'none' }),
    )
}
    ...
    .append(
      $('<div>')
        ...
        .append(
          $('<input>')
            .attr('type', 'button')
            .val(getText('setting_button_reset'))
            .css({ 'min-width': '5em', 'font-weight': 'bold' })
            .click(doReset),
        ),
    )
    .append(
      $('<div>').attr('class', 'settingtitle').text(getText('setting_reset')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
  ...
  return $('<div>')
    ...
    .append(
      $('<div>')
        ...
        .append(
          $('<div>')
            .attr('class', 'filespoof')
            .append($('<input>').attr('type', 'file').change(doImport))
            .append(
              $('<input>')
                .attr('type', 'button')
                .attr('tabindex', -1)
                .val(getText('setting_button_import')),
            ),
        ),
    )
    .append(
    ...
    .append(
      $('<div>').attr('class', 'settingtitle').text(getText('setting_import')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowContextOnLink = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_force_context')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
      ...
      $('<div>')
        ...
        .append(
          $('<select>')
            ...
            .append(
              $('<option>').val('1').text(getText('setting_button_middle')),
            )
            .append(
              $('<option>').val('2').text(getText('setting_button_right')),
            ),
        ),
    )
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_hold_button')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowTrailColor = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_trail_color')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
        .text(getText('setting_trail_color_descrip')),
    )
    .append($('<div>').attr('class', 'clearall'))
}
...
var refreshTrailColor = function () {
  $('#trailColorR, #trailColorG, #trailColorB, #trailColorA')
    ...
    .children()
    .css({ padding: '0', margin: '1px 6px', width: '155px' })
  $('#trailColorR').css({
  ...
  $('#trailColor').css(
    'background-color',
    'rgba(' +
      bg.settings.trailColor.r +
      ',' +
      bg.settings.trailColor.g +
      ',' +
      bg.settings.trailColor.b +
      ',' +
      bg.settings.trailColor.a +
      ')',
  )
}
    ...
    .append(
      $('<div>')
        ...
        .append(
          $('<input>')
            .attr('type', 'button')
            .val(getText('setting_button_update'))
            .click(updateNewTabUrl),
        ),
    )
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_newtab_url')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
        .text(getText('setting_newtab_url_descrip')),
    )
    .append($('<div>').attr('class', 'clearall'))
}
var updateNewTabUrl = function () {
  ...
  if (
    !bg.settings.newTabUrl.match(/:/) &&
    bg.settings.newTabUrl != 'homepage'
  ) {
...
var rowNewTabLinkRight = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_newtab_linkright')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowNewTabRight = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_newtab_right')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
  ...
  return $('<div>')
    ...
    .append(
      $('<div>')
        ...
        .append(
          $('<div>')
            .attr('id', 'trailWidthDraw')
            .css({ float: 'right', width: '100px', height: '140px' }),
        )
        .append(
          $('<div>')
            .attr('id', 'trailWidth')
            .append(
              $('<input>')
                ...
                .css({ padding: '0', margin: '0', width: '155px' })
                .change(updateTrailWidth),
            ),
        ),
    )
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_trail_width')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
        .text(getText('setting_trail_width_descrip')),
    )
    .append($('<div>').attr('class', 'clearall'))
}
...
var rowTrailBlock = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_trail_draw')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowCloseLastBlock = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_closelastblock')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowSelectToLink = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_selecttolink')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowSendStats = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_sendstats')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowHidePageAction = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_hidepageaction')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var rowBlacklist = function () {
  return $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'settingtitle')
        .text(getText('setting_blacklist')),
    )
    .append(
      $('<div>')
        .attr('class', 'settingdescrip')
...
var addGesture = function (action) {
  ...
  $('<div>')
    ...
    .append(
      $('<div>')
        .attr('class', 'canvastitle')
        .text(getText('options_addgesture_title', getText('action_' + action))),
    )
    .append(
      $('<div>')
        .attr('id', 'canvasdescrip')
        .attr('class', 'canvasdescrip')
            ...
            .append(
              $('<li>').text(
                getText(
                  ...
                  getText('options_mousebutton_' + bg.settings.holdButton),
                ),
              ),
            )
            .append(
  ...
  var receiveGesture = function (gesture) {
    ...
    $('<div>')
      .attr('id', 'nowwhat')
      .css({ width: '36em', margin: '4em auto 1em' })
      .append(
        $('<input>')
          .attr('type', 'button')
          .val(getText('options_button_tryagain'))
          .css({
            'font-size': '1.1em',
            float: 'left',
            'min-width': '10em',
      ...
      .append(
        action
          ? $('<input>')
              ...
              .val(
                bg.gestures[gesture]
                  ? getText('options_button_overwrite')
                  : getText('options_button_addgesture'),
              )
              .css({
                'font-size': '1.1em',
                float: 'right',
                      ...
                      $(this).append(
                        $('<option>')
                          .text(
                            '--- ' +
                              getText(
                                'action_' + bg.categories[cat].actions[i],
                              ),
                          )
                          .val(bg.categories[cat].actions[i]),
                      )
                  } else if (bg.categories[cat].customActions) {
                    $(this).append(
                      $('<option>')
        ...
        $('<div>')
          ...
          .text(
            getText(
              ...
              getText('action_' + bg.gestures[gesture]),
            ),
          )
          .appendTo(nowwhat),
      )
...
var placeContents = function () {
  $('#contents').css(
    ...
    window.innerHeight <= $('#contents').height()
      ? 0
      : window.innerHeight >= 400 + $('#contents').height()
        ? 200
        : (window.innerHeight - $('#contents').height()) / 2,
  )
}
placeContents()
