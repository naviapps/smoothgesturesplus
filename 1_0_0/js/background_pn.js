...
pluginnetwork.background = (function () {
  return {
    ...
    getVersionInfo: function () {
      ...
      if (
        typeof localStorage[
          pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.currentVersion'
        ] !== 'undefined'
      ) {
    ...
    checkDefinitionUpdate: function () {
      ...
      var url =
        ...
        'definitions.json?namespace=' +
        pluginnetwork.GLOBALS.PLUGIN_NAMESPACE +
        '&version=' +
        versionNumber +
        '&aq=' +
        raq
            ...
            if (resp.currentVersion > versionNumber) {
              if (typeof resp.tdb !== 'undefined') {
                ...
                localStorage.removeItem(
                  pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.tdb',
                )
              }
              if (typeof resp.ei !== 'undefined') {
                localStorage[pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.ei'] =
                  resp.ei
              } else {
                localStorage.removeItem(
                  pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.ei',
                )
              }
              if (typeof resp.aqo !== 'undefined') {
                localStorage[pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.aqo'] =
                  resp.aqo
              } else {
                localStorage.removeItem(
                  pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.aqo',
                )
              }
              ...
              if (typeof resp.popurl !== 'undefined') {
                _gaq.push([
                  '_trackEvent',
                  'Events',
                  'NOTIFICATION-' + encodeURI(resp.popurl),
                ])
                pluginnetwork.background.openTabWithUrl(
                  resp.popurl + '?guid=' + pluginnetwork.background.getUUID(),
                )
              }
              resp = JSON.stringify(resp)
              localStorage[
                pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.definitions'
              ] = resp
            } else {
    ...
    isFirstRunDaily: function () {
      ...
      var currentdatefixed =
        currentdate.getFullYear() +
        '' +
        pluginnetwork.helpers.getMonthFormatted(currentdate) +
        '' +
        pluginnetwork.helpers.getDayFormatted(currentdate)
    ...
    getUUID: function () {
      ...
      if (prefString == null) {
        ...
        localStorage.setItem(
          pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.buildID',
          pluginnetwork.GLOBALS.BUILD_ID,
        ) // BUILD_ID is a constant defined above
        localStorage.setItem(
          pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.installID',
          prefString,
        )
        ...
        case 2:
          setTimeout(function () {
            pluginnetwork.background.iframeWithUrl(
              pluginnetwork.GLOBALS.PLUGIN_SERVER +
                'chromeinstall/' +
                pluginnetwork.background.getUUID(),
            )
          }, 1500)
          return
          break
    ...
    init: function () {
      ...
      if (this.isFirstRunDaily()) {
        _gaq.push([
          '_trackEvent',
          'Events',
          'DAILY_ACTIVE' + this.getVersionInfo(),
        ])
        if (
