      ...
      .append(
        $('<input>')
          ...
          .click(function () {
            ...
            $.ajax({
              url: 'http://smoothgesturesapp.com/translate.php',
              type: 'post',
              data: {
                key: 'smooth',
                lang: language,
...
$('body').append(
  $(
    '<h1 id="translatetitle"><img src="im/icon48.png"/> Smooth Gestures: Translate</h1>',
  ),
)
  ...
  for (id in src) {
    if (id != '_')
      words.push({
        ...
        src: src[id],
        res:
          r[id] && r[id].update && r[id].update > src[id].update
            ? r[id]
            : undefined,
      })
    delete r[id]
  }
  for (id in r) words.push({ id: id, src: undefined, res: r[id] })
...
var buildpage = function () {
  ...
  var cats = []
  for (c in table) cats.push(c)
  cats.sort()
  for (i = 0; i < cats.length; i++) root.append(table[cats[i]])
  $('body').append(root)
}
