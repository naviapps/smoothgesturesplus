...
type ValidGestures = ValidLineGestures & {
...
export type SmoothGesturesProps = {
  ...
  sendMessage: (message: ContentMessage) => Promise<void>;
  onMessage: {
    addListener: (callback: (message: BackgroundMessage) => void) => void;
    removeListener: (callback: (message: BackgroundMessage) => void) => void;
  };
...
export function SmoothGestures({
  ...
  callback,
}: SmoothGesturesProps): React.ReactNode {
  ...
  useEffect(() => {
     ...
     */
    // TODO
    const receiveMessage = (message: BackgroundMessage): void => {
      /*
      if (message.action) {
