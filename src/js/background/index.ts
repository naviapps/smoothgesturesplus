...
defaults = {};
defaults.settings = JSON.stringify({
  holdButton: 2,
  contextOnLink: false,
...
defaults.gestures = JSON.stringify({
  U: 'new-tab',
  lU: 'new-tab-link',
  D: 'toggle-pin',
  L: 'page-back',
...
const initConnectTab = (port) => {
  ...
  contents[id].onMessage.addListener(
    function (id, mess) {
          ...
          ? () => {
              chrome.tabs.query({ active: true, lastFocusedWindow: true }, (e) => {
                if (chainGesture && e.length)
                  for (id in ((chainGesture.tabId = e[0].id), contents))
                    e[0].id == contents[id].detail.tabId &&
      ...
      if (mess.nativeport && mess.nativeport.rightclick) {
        ...
        if (m) {
          m.postMessage({
            click: { x: mess.nativeport.rightclick.x, y: mess.nativeport.rightclick.y, b: 2 },
            timestamp: Date.now(),
          });
  ...
  chainGesture &&
    ...
    (tab.active
      ...
      : (clearTimeout(chainGesture.timeout), (chainGesture = null)));
  ...
  for (let a = 0; settings.blacklist && a < settings.blacklist.length; a += 1)
    new RegExp(`^(.+\\.)?${settings.blacklist[a].replace('.', '\\.')}$`).test(i) &&
      (o.enable = false);
  contents[id].postMessage(o), refreshPageAction(tab.id);
};
...
const n = (e) => {
  if (f.active != e) {
    for (id in contents)
      f.active == contents[id].detail.tabId && contents[id].postMessage({ windowBlurred: true });
...
const refreshPageAction = (tabId) => {
  getTabStatus(tabId, (stat) => {
    if (stat === 'unable') {
      ...
      chrome.pageAction.setTitle({ tabId, title: "Gestures don't work. Reload" });
    } else {
      chrome.pageAction.setIcon({
        tabId,
        path: chrome.runtime.getURL('/img/pageaction.png'),
