  ...
  $('#trail_width input[type=range]').on('change', () => {
    ...
    $('#reset.setting .button').on('click', () => {
      confirm(c('setting_warning_reset')) &&
        chrome.runtime.getBackgroundPage((e) => {
          i({ gestures: JSON.parse(e.defaults.gestures) }), h(), v();
        });
    });
  const a = [
    'gestures',
...
const r = (t) => {
  if (confirm('Delete this custom action?')) {
    for (const e in (delete l.customactions[t], i({ customactions: l.customactions }), l.gestures))
      l.gestures[e] == t && p(e);
    chrome.runtime.getBackgroundPage((e) => {
      delete e.contexts[t];
    }),
...
var d = {
  ...
  open(e) {
    if (window.SG && !window.SG.callback) {
      ...
      $('#canvasdescrip').css({ display: 'table' });
      $('#nowwhat').css({ display: 'none' })((window.SG.callback = d.gesturecallback));
    }
  },
};
