document.title =
  chrome.i18n.getMessage('name') + ': ' + chrome.i18n.getMessage('status_title')
$('body')
  .append(
    $(
      '<h1 id="optionstitle"><img src="im/icon48.png"/> ' +
        ...
        ': ' +
        chrome.i18n.getMessage('status_title') +
        '</h1>',
    ),
  )
  .append(
    '<div id=optionslink><a href="options.html">' +
      chrome.i18n.getMessage('status_link_options') +
      '</a></div>',
  )
  .append(
    '<div id=legend><input class="reloadall" type="button" value="' +
      chrome.i18n.getMessage('status_button_reloadall') +
      '"><div id="lred">' +
      chrome.i18n.getMessage('status_legend_broken') +
      '</div><div id="lgray">' +
      chrome.i18n.getMessage('status_legend_blocked') +
      '</div><div id="lyellow">' +
      chrome.i18n.getMessage('status_legend_loading') +
      '</div><div id="lgreen">' +
      chrome.i18n.getMessage('status_legend_working') +
...
var generateTab = (state, length) => {
  ...
  if (state.root) tab.css({ 'background-color': '#aaeeaa' })
  else if (state.tabStatus == 'loading')
    tab.text('...').css({ 'background-color': '#eeeeaa' })
  else if (!state.goodurl) tab.css({ 'background-color': '#cccccc' })
  else
    tab.css({ 'background-color': '#eeaaaa' }).prepend(
      $(
