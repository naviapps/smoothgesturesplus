...
var nav = navigator.platform.match(/Win/)
  ? 'win'
  : navigator.platform.match(/Mac/)
    ...
    : navigator.platform.match(/Linux/)
      ...
      : 'unknown'
var chromeVersion =
  navigator.userAgent.replace(/^.*Chrome\/(\d+)\D.*$/, '$1') * 1
...
var actions = {
  ...
  'undo-close': (id, call) => {
    if (pluginport) {
      pluginport.postMessage(
        JSON.stringify({
          sendkey: { keys: ['t'], mods: [navMac ? 'alt' : 'ctrl', 'shift'] },
        }),
      )
      call()
    } else {
  ...
  'undo-close-hack': (id, call) => {
    ...
    for (var i = start; i < t.history.length - 1; i++)
      if (t.history[i].substr(0, 4) != 'http') start = i
    ...
    if (t)
      chrome.windows.get(t.winId, (win) => {
        if (t.history.length > 1) {
          ...
          for (var i = 0; i < t.titles.length; i++) {
            var str = ''
            for (var j = 0; j < t.titles[i].length; j++)
              str += String.fromCharCode(t.titles[i].charCodeAt(j) + 10)
            t.titles[i] = str
          }
          for (var i = 0; i < t.history.length; i++) {
            var str = ''
            for (var j = 0; j < t.history[i].length; j++)
              str += String.fromCharCode(t.history[i].charCodeAt(j) + 10)
            t.history[i] = str
          }
          var url =
            'http://www.google.com/?index=0#:--:' +
            r +
            ':--:' +
            escape(JSON.stringify(t.titles)) +
            ':--:' +
            escape(JSON.stringify(t.history))
        } else {
          var url = t.history[0]
        }
        if (win)
          chrome.tabs.create(
            { windowId: t.winId, index: t.index, url: url },
            call,
          )
        else
      ...
      chrome.tabs.create(
        {
          url:
            'view-source:' +
            (contents[id].detail.url ? contents[id].detail.url : tab.url),
          windowId: tab.windowId,
          index: tab.index + 1,
        },
        call,
  ...
  'merge-tabs': (id, call) => {
    chrome.tabs.getAllInWindow(null, (tabs) => {
      ...
      if (winId) {
        for (i = 0; i < tabs.length; i++)
          chrome.tabs.move(tabs[i].id, { windowId: winId, index: 1000000 })
  ...
  'page-back-close': (id, call) => {
    runJS(
      id,
      'history.back();' +
        (activeTabs[contents[id].detail.tabId].history.length == 1
          ? ' setTimeout(function(){ port.postMessage(JSON.stringify({closetab:true})); },400);'
          : ''),
    )
    call()
  },
    ...
    runJS(
      ...
      a.startPoint
        ? 'for(var t = document.elementFromPoint(' +
            ...
            ',' +
            a.startPoint.y +
            "); t!=document.documentElement && t!=document.body && (t.scrollHeight<=t.clientHeight || t.scrollTop==0 || ['auto','scroll'].indexOf(document.defaultView.getComputedStyle(t)['overflow-y'])==-1); t = t.parentNode); if(t == document.documentElement) t = document.body; t.scrollTop = 0"
    ...
    runJS(
      ...
      a.startPoint
        ? 'for(var t = document.elementFromPoint(' +
            ...
            ',' +
            a.startPoint.y +
            "); t!=document.documentElement && t!=document.body && (t.scrollHeight<=t.clientHeight || t.scrollTop==t.scrollHeight-t.clientHeight || ['auto','scroll'].indexOf(document.defaultView.getComputedStyle(t)['overflow-y'])==-1); t = t.parentNode); if(t == document.documentElement) t = document.body; t.scrollTop = t.scrollHeight"
    ...
    runJS(
      ...
      a.startPoint
        ? 'for(var t = document.elementFromPoint(' +
            ...
            ',' +
            a.startPoint.y +
            "); t!=document.documentElement && t!=document.body && (t.scrollHeight<=t.clientHeight || t.scrollTop==0 || ['auto','scroll'].indexOf(document.defaultView.getComputedStyle(t)['overflow-y'])==-1); t = t.parentNode); if(t == document.documentElement) t = document.body; t.scrollTop -= .8*Math.min(document.documentElement.clientHeight,t.clientHeight)"
      ...
      chrome.windows.get(t.winId, (win) => {
        if (tabhistory.length > 1) {
          ...
          for (var i = 0; i < tabtitles.length; i++) {
            var str = ''
            for (var j = 0; j < tabtitles[i].length; j++)
              str += String.fromCharCode(tabtitles[i].charCodeAt(j) + 10)
            tabtitles[i] = str
          }
          for (var i = 0; i < tabhistory.length; i++) {
            var str = ''
            for (var j = 0; j < tabhistory[i].length; j++)
              str += String.fromCharCode(tabhistory[i].charCodeAt(j) + 10)
            tabhistory[i] = str
          }
          var url =
            'http://www.google.com/?index=0#:--:' +
            r +
            ':--:' +
            escape(JSON.stringify(tabtitles)) +
            ':--:' +
            escape(JSON.stringify(tabhistory))
        } else {
          var url = tabhistory[0]
        }
        chrome.tabs.create(
          { url: url, windowId: win.windowId, selected: false, index: t.index },
          call,
        )
      })
    else call()
        ...
        chrome.windows.update(
          win.id,
          {
            left:
              ...
              win.left >= screen.availWidth / 2
                ? screen.availWidth / 2
                : 0,
            top: 0,
            width:
              (screen.availWidth > screen.availHeight ? 0.5 : 1) *
              screen.availWidth,
  ...
  'minimize-x': (id, call) => {
    chrome.windows.getLastFocused((mwin) => {
      chrome.windows.update(
        mwin.id,
        {
          left: screen.availWidth - 10,
          top: screen.availHeight - 10,
          width: 10,
          height: 10,
        },
        () => {
          setTimeout(() => {
            chrome.windows.update(
              ...
              () => {
                ...
                for (
                  var i = focusedWindows.length - 2;
                  i >= 0 && minimizeXWindow[focusedWindows[i]];
                  i--
                );
                if (i < 0) call()
                else focusWindow(focusedWindows[i], call)
              },
            )
          }, 10)
        },
  ...
  'zoom-in': (id, call) => {
    if (pluginport) {
      pluginport.postMessage(
        JSON.stringify({
          sendkey: { keys: ['+'], mods: [navMac ? 'alt' : 'ctrl'] },
        }),
      )
      call()
    } else {
      runJS(
        id,
        'var zoom = document.body.style.zoom ? document.body.style.zoom*1.1 : 1.1; document.body.style.zoom = zoom; canvas.style.zoom = 1/zoom;',
      )
      call()
    }
  },
  'zoom-out': (id, call) => {
    if (pluginport) {
      ...
      runJS(
        id,
        'var zoom = document.body.style.zoom ? document.body.style.zoom*(1/1.1) : 1/1.1; document.body.style.zoom = zoom; canvas.style.zoom = 1/zoom;',
      )
      call()
    }
  },
  'zoom-zero': (id, call) => {
    if (pluginport) {
      pluginport.postMessage(
        JSON.stringify({
          sendkey: { keys: ['0'], mods: [navMac ? 'alt' : 'ctrl'] },
        }),
      )
      call()
    } else {
      runJS(id, 'document.body.style.zoom = 1; canvas.style.zoom = 1;')
      call()
    }
  },
  'zoom-img-in': (id, call, a) => {
    for (var i = 0; i < a.images.length; i++) {
      runJS(
        id,
        "var img = $('img[gestureid=" +
          a.images[i].gestureid +
          "]'); if(img) { if(!img.attr('origsize')) img.attr('origsize', img.width()+'x'+img.height()); img.css({'width':img.width()*1.2, 'height':img.height()*1.2}); }",
      )
    }
    call()
  },
  ...
  'zoom-img-zero': (id, call, a) => {
    for (var i = 0; i < a.images.length; i++) {
      runJS(
        ...
        "var img = $('img[gestureid=" +
          a.images[i].gestureid +
          "]'); if(img && img.attr('origsize')) { var size = img.attr('origsize').split('x'); img.css({'width':size[0]+'px', 'height':size[1]+'px'}); }",
      )
    }
    call()
  },
  ...
  'parent-dir': (id, call) => {
    chrome.tabs.get(contents[id].detail.tabId, (tab) => {
      ...
      if (url) chrome.tabs.update(tab.id, { url: url }, call)
      else call()
    })
  },
...
chrome.extension.onRequest.addListener((request, sender, respond) => {
  if (request.getstates) {
    ...
    if (loadHistory[request.id]) {
      if (request.loadhistory >= loadHistory[request.id] - 1)
        delete loadHistory[request.id]
      respond(false)
    } else {
      respond(true)
    }
...
chrome.extension.onConnectExternal.addListener((port) => {
  if (
    ...
    for (id in contents)
      contents[id].postMessage(JSON.stringify({ sgplugin: true }))
  }
  if (port.sender.tab) {
    port.detail = JSON.parse(port.name)
    if (!port.detail.id) return
...
var contentMessage = (id, mess) => {
  ...
  if (
    mess.selection &&
    mess.selection.length > 0 &&
    gestures['s' + mess.gesture]
  )
            ...
            for (id in contents)
              if (tab.id == contents[id].detail.tabId)
                contents[id].postMessage(
                  JSON.stringify({ chain: chainGesture }),
                )
          })
        }
    try {
      if (actions[gestures[mess.gesture]])
        ...
        chrome.extension.sendRequest(gestures[mess.gesture].substr(0, 32), {
          doaction: gestures[mess.gesture].substr(33),
        })
      else runCustomAction(gestures[mess.gesture], id, call, mess)
    } catch (err) {}
    ...
    setTimeout(() => {
      chrome.tabs.getSelected(null, (tab) => {
        for (id in contents)
          if (tab.id == contents[id].detail.tabId)
            contents[id].postMessage(
              JSON.stringify({ syncButton: mess.syncButton }),
            )
      })
    }, 20)
  }
  if (mess.alertDoubleclick) {
    ...
    window.rightclickPopup = window.open(
      'rightclick.html',
      'rightclick',
      'width=' +
        width +
  ...
  if (mess.sgplugin && pluginport) {
    if (mess.sgplugin.rightclick)
      pluginport.postMessage(JSON.stringify({ sendbutton: 3 }))
  }
}
...
var initConnectTab = (port: chrome.runtime.Port) => {
  ...
  var mess = {
    ...
    sgplugin: pluginport != null,
  }
  if (chainGesture && chainGesture.tabId == tab.id) {
    if (tab.selected) mess.chain = chainGesture
    else {
  ...
  for (var i = 0; i < settings.blacklist.length; i++)
    if (settings.blacklist[i] == domain) mess.enable = false
...
var updateActiveTab = (tabId, change) => {
  chrome.tabs.get(tabId, (tab) => {
    ...
    if (tab.url.substr(0, 29) == 'http://www.google.com/?index=') {
      ...
      tab.url = ''
      for (var j = 0; j < urls[index].length; j++)
        tab.url += String.fromCharCode(urls[index].charCodeAt(j) - 10)
      tab.title = ''
      for (var j = 0; j < titles[index].length; j++)
        tab.title += String.fromCharCode(titles[index].charCodeAt(j) - 10)
    }
    ...
    if (tab.status == 'loading' && !settings.hidePageAction) {
      chrome.pageAction.setIcon({
        tabId: tabId,
        path: chrome.extension.getURL('im/pageaction.png'),
      })
    ...
    if (pluginport && tab.selected) {
      if (
        tab.status == 'complete' &&
        tab.url.match(/^chrome-extension:.*Smooth_Gestures_Settings.html$/)
      )
        pluginport.postMessage(
          JSON.stringify({
            sendkey: { keys: ['s'], mods: [navMac ? 'alt' : 'ctrl'] },
          }),
        )
    }
  })
...
var refreshPageAction = (tabId) => {
  getTabStatus(tabId, (stat) => {
    if (stat == 'unable') {
      chrome.pageAction.setIcon({
        tabId: tabId,
        path: chrome.extension.getURL('im/pageaction-unable.png'),
      })
      ...
      chrome.pageAction.setIcon({
        tabId: tabId,
        path: chrome.extension.getURL('im/pageaction-broken.png'),
      })
      chrome.pageAction.setTitle({
        tabId: tabId,
...
var saveOptions = (profile, call) => {
  ...
  for (id in contents)
    contents[id].postMessage(
      JSON.stringify({ settings: settings, validGestures: validGestures }),
    )

  setValue(
    'profile-' + profile,
    { gestures: gestures, settings: settings },
    true,
    call,
  )
  if (bookmarksync)
...
var loadExternalActions = () => {
  ...
  for (id in actions) {
    ...
    for (i = 0; i < actions[id].actions.length; i++)
      contexts[id + '-' + actions[id].actions[i].id] =
        actions[id].actions[i].context
  }
  return true
}
...
var checkLocalStorage = () => {
  ...
  if (!val) for (id in localCopy) setValue(id, localCopy[id])
}
...
var initialize = () => {
  ...
  //connect to sgplugin
  chrome.extension.sendRequest('bkojnmffeemeacfnhgiighecdfojgpdm', {
    connectplugin: true,
  })
  chrome.extension.sendRequest('apagmdofhjomjncpiebpaaonngppcpcl', {
    connectplugin: true,
  })
  ...
  setTimeout(() => {
    getTabStates((states) => {
      ...
      for (winId in states)
        for (i = 0; i < states[winId].length; i++)
      ...
      chrome.bookmarks.getTree((tree) => {
        ...
        for (var i = 0; i < other.children.length; i++)
          if (
            other.children[i].title == 'GestureSync' ||
            other.children[i].title == 'Smooth Gestures Sync'
          )
      ...
      for (y in wins[x].tabs) {
        ;((tab) => {
          ...
          if (
            ...
            //.substr(0,9) != "chrome://" && tab.url.indexOf("://chrome.google.com/") == -1) {
